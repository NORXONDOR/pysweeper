...
class Tile():
    def __init__(self, is_mine):
        ...
        Args:
            (bool) is_mine: Determines if Tile holds a mine.
        """
...
class Board():
    def __init__(self, x_dim=10, y_dim=10, mine_prop=0.1):
        """
        Creates a mineClearer Board.
        
        Args:
            (int) x_dim: Width of the board, dim < 100.
            (int) y_dim: Height of the board, dim < 100.
            (float) mine_prop: Proportion of tiles that are mines.
        """
        self.x_axis = x_dim
        self.y_axis = y_dim
        self.gamestate = 0
        ...
        # Create a randomly ordered array of mine locations.
        tile_count = self.x_axis*self.y_axis
        self.covered_count = tile_count
        
        self.mine_count = floor(tile_count*mine_prop) 
        safe_count = tile_count-self.mine_count
        
        mine_array = [True] * self.mine_count + [False] * safe_count
        shuffle(mine_array)
        ...
        for y in range(self.y_axis):
            ...
            for x in range(self.x_axis):
                self.tile_matrix[y].append(Tile(is_mine=mine_array[y*x_dim+x]))

        # Get coordinates, store all adjacent Tiles and calcualte adjacent mines for each Tile.
        for y in range(self.y_axis):
            for x in range(self.x_axis):
        ...
        for y in range(self.y_axis):
            ...
            for x in range(self.x_axis):
                if self.tile_matrix[y][x].covered:
                    ...
                    if self.tile_matrix[y][x].is_mine:
                        out_string += '* '
                    else:
                        if self.tile_matrix[y][x].adj_mines == 0:
                            out_string += '  '
                        else:
                            out_string += str(self.tile_matrix[y][x].adj_mines) + ' '
            out_string += '\n'
        print(out_string)
    ...
    def uncover(self, x, y):
        ...
        if target_tile.covered:
            ...
            # If Tile has 0 adjacent mines, uncover() recursively until all adjacent safe unflagged Tiles discovered.
            if target_tile.adj_mines == 0:
                for tile in target_tile.adj_tiles:
                    if not tile.is_mine and not tile.flagged:
                        x, y = tile.coords
                        self.recursed = True
                        self.uncover(x, y)
                        self.recursed = False
            ...
            if target_tile.is_mine:
                ...
                # Allow all mines to be displayed.
                for y in range(self.y_axis):
                    for x in range(self.x_axis):
                        target_tile = self.tile_matrix[y][x]
            ...
            # Win condition.
            if board.covered_count == board.mine_count:
                board.gamestate = 1
...
if __name__ == "__main__":
    while True:
        clear()
        print(f"mineClearer v{VERSION} by NORXONDOR\n")
        print("Select difficulty:")
        print("0 : Easy - 9x9 / 10 mines")
        print("1 : Medium - 16x16 / 40 mines")
        print("2 : Hard - 30x16 / 99 mines")
        print("q : Quit game")

        match input("> "):
            case '0':
                board = Board(x_dim=9, y_dim=9, mine_prop=0.12346)
                break
            
            case '1':
                board = Board(x_dim=16, y_dim=16, mine_prop=0.15625)
                break
            
            case '2':
                board = Board(x_dim=30, y_dim=16, mine_prop=0.20625)
                break
    ...
    while True:
        ...
        if board.gamestate == -1:
            ...
            print("  X X")
            print("  -u-\n")
            print(f"mine found at ({x},{y}). You lose!")
            break
        elif board.gamestate == 1:
            # Win condition met.
            print("  O O")
            print("   U \n")
            print(f"All mines located. You win!")
            break